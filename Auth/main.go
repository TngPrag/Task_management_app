...
func pmain() {
	...
	policy := Policy{Sub: "customer", Obj: "luwa/api/v0.1/customer", Act: "GET"}
	if err := enforceWrapper.CreatePolicy(core.Policy(policy)); err != nil {
		fmt.Errorf("create policy error", err)
	}
	//read policies by subject
	policies, err := enforceWrapper.ReadPoliciesForSubject("driver")
	if err != nil {
		fmt.Errorf("read policy error", err)
	}
	fmt.Println(policies)
	// list all policies
	all, err := enforceWrapper.GetAllPolicy()
	fmt.Println(all)
	role := core.Role{User: "Tsegay", Role: "super-admin"}
